...
void blowfish_init(blowfish_t* bf, uint8_t* key, unsigned len)
{
    bf->XL = 0;
    bf->XR = 0;
    memcpy(bf->S, _blowfish_S, sizeof(_blowfish_S));

    for (unsigned i = 0; i < 18; i++)
    {
        bf->P[i] = _blowfish_P[i] ^ 
            _bswap32(*(uint32_t*)(key+(i*4)%len));
    }
